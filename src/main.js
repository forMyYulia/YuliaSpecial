...
window.addEventListener('scroll', (e) => {
    underText.style.transform = `scale(${Math.min(window.scrollY / 15, 150) / 100}) translateY(-${Math.min(Math.max(window.scrollY/20-750, 0), 135)}px)`
    box.style.transform = `scale(${Math.max(180-window.scrollY/15, 80)/100})`// rotate(${Math.max(Math.min((window.scrollY-2500)/80, 20), 0)}deg)`;
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (i < 2)
            subSides[i].style.transform = `translateY(${Math.min(window.scrollY/6, 289)}px) translateX(${i === 1 || i === 2 ? '': '-'}${Math.max(Math.min((window.scrollY-8500)/120, 19), 0)*10}px)`;
    }
    for (let j = 2; j < 6; j++) {
        subSides[j].style.transform = `translateX(${j < 4 ? '-': ''}${Math.min(window.scrollY/6, 289)}px) translateY(${j === 3 || j === 4 ? '': '-'}${Math.max(Math.min((window.scrollY-4500)/120, 19), 0)*10}px)`;
    }
    for (let j = 6; j < 9; j++) {
        subSides[j].style.transform = `translate${j < 8? 'X' : 'Y'}(${j === 7 ? '-': ''}${Math.min(window.scrollY/6, 289)+Math.max(Math.min((window.scrollY-13000)/120, 13), 0)*10}px)`;
    }
    ...
    bottomTexts[0].style.opacity = `${100-Math.min(Math.max(window.scrollY/20-150, 0), 100)}%`;
    bottomTexts[1].style.opacity = `${100-Math.min(Math.max(150 - Math.max(window.scrollY/20-200, 0), 0) + 2*Math.max(window.scrollY/20-400, 0), 100)}%`;
    bottomTexts[2].style.opacity = `${100-Math.min(Math.max(150 - Math.max(window.scrollY/20-400, 0), 0) + 2*Math.max(window.scrollY/20-600, 0), 100)}%`;
    bottomTexts[3].style.opacity = `${100-Math.min(Math.max(150 - Math.max(window.scrollY/20-600, 0), 0), 100)}%`;
})
